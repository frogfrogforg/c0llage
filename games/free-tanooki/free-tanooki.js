...
function Update() {
  ...
  if (currentObjectSet === 1 && pipeFrame === 0 && lastPipe > 0 && lastPipe !== 192) {
    if (window.top.addEvent != null && !finishedLevel) {
      d.Events.raise('mario.exitlevel')
      finishedLevel = true
    }
  }
  ...
  if (ypos > 235 && xpos > 150 && xpos < 160 && !gotToTop) {
    gotToTop = true
    d.Events.raise('mario.freesalad')
  }
...
d.Events.listen('pressed-me-up', (event) => {
...
d.Events.listen('juice.appeared', () => {
...
d.Events.listen('juice.outbucket', () => {
  if (!hasBucketControl) return
